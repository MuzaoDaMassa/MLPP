...
// Declare namepsace which will contain all methods
// that are needed for Data Analysis
namespace DataAnalysis
{
    ...
    Mat2d<std::string> read_csv_file(const std::string& filePath)
    {
    ...
    // Method to convert string to differnt data type, returns new 2d matrix with
    // passed type
    template <typename T>
    Mat2d<T> matrix_converter(const Mat2d<std::string>& stringMatrix)
    {
    ...
    template <typename T>
    void matrix_formatter(Mat2d<T>& dataMatrix, Formatter f, const int& toBeRemoved)
    {
        if (!dataMatrix.empty()) {
            if (f == COLUMN) {
                if (toBeRemoved >= 0) {
                ...
                // Display error to inform invalid index to remove
                std::cerr << "Error: Invalid index of row to remove"
                          << std::endl;
            } 
            else {
    ...
    // rocToRemove = rows or columns to remove, vector will hold all rows or columns
    // to remove
    template <typename T>
    void matrix_formatter(Mat2d<T>& dataMatrix, Formatter f, const std::vector<int>& rocToRemove)
    {
        if (!dataMatrix.empty()) {
            if (f == COLUMN) {
    ...
    // racToRemove = rows and columns to remove, outside vector will hold rows to
    // remove while inside will hold columns
    template <typename T>
    void matrix_formatter(Mat2d<T>& dataMatrix, Formatter f, const Mat2d<int>& racToRemove)
    {
        if (!dataMatrix.empty()) {
            if (f == ROWANDCOLUMN) {
                ...
                std::cerr << "Error: No rows and columns to remove"
                          << std::endl;
            }
            // Display error to inform wrong formatter enum was passed
            std::cerr << "Error: Wrong formatter passed (2nd Paramater)"
    ...
    template <typename T>
    void matrix_formatter(Mat2d<T>& dataMatrix, Formatter f, const int& indexToAdd,
        ...
        if (!dataMatrix.empty()) {
            if (f == COLUMN) {
                ...
                if (indexToAdd >= 0 && indexToAdd <= dataMatrix.size()) {
                    dataMatrix.insert(dataMatrix.begin() + indexToAdd,
                                      dataToAdd);
                    return;
                }
    ...
    // Method to search data matrix for first appearance of desired element, return
    // first position found
    template <typename T>
    std::vector<int> find(const Mat2d<T>& dataMatrix, const T& desiredElement)
    {
    ...
    // Method to search data matrix by position, return element at requqested
    // position on 2d Matrix
    template <typename T>
    T find_by_pos(const Mat2d<T>& dataMatrix, std::vector<int>& pos)
    {
        T element;
    ...
    template <typename T>
    Mat2d<int> findAll(const Mat2d<T>& dataMatrix, const T& desiredElemet)
    {
        // Creat position vector
        Mat2d<int> pos;
    ...
    template <typename T>
    void displayAll(const Mat2d<T>& dataMatrix)
    {
        if (!dataMatrix.empty()) {
            for (auto &row : dataMatrix) {
                for (auto &cell : row) {
    ...
    template <typename T>
    void displayRows(const Mat2d<T>& dataMatrix, const std::vector<int>& rowsToDisplay)
    {
    ...
    template <typename T>
    void displayColumns(const Mat2d<T>& dataMatrix, const std::vector<int>& colsToDisplay)
    {
    ...
    template <typename T>
    void displayHead(const Mat2d<T>& dataMatrix, int rowsToDisplay = 5)
    {
        if (!dataMatrix.empty()) {
    ...
    template <typename T>
    void displayBottom(const Mat2d<T>& dataMatrix, int rowsToDisplay = 5)
    {
        if (!dataMatrix.empty()) {
            if (rowsToDisplay >= 0 && rowsToDisplay < dataMatrix.size() - 1) {
...
// Further namespaces to be implemented